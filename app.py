...
def generate_report(weights, metrics):
    ...
    # FIXED: fpdf2 returns bytes directly
...
try:
    ...
    c2.metric("Cornish-Fisher VaR", f"{cf_var:.2%}")
    c3.metric("Max Drawdown", f"{((p_cum - p_cum.cummax()) / p_cum.cummax()).min():.1%}")
    c4.metric("Ann. Volatility", f"{(p_returns.std()*np.sqrt(252)):.1%}")
    ...
    if stress_res:
        st.plotly_chart(px.bar(pd.DataFrame.from_dict(stress_res, orient='index'), template="plotly_dark", labels={'index': 'Crisis', 'value': 'Return'}))

    # --- DOWNLOAD REPORT ---
    report_data = generate_report(clean_weights, {"Sharpe": f"{sharpe:.2f}", "Beta": f"{exposures['Beta']:.2f}", "VaR": f"{cf_var:.2%}"})
