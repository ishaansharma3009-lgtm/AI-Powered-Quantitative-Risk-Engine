...
def get_clean_data(tickers, start):
    ...
    try:
        ...
        for t in all_tickers:
            try:
                if t in raw_data.columns.get_level_values(0):  # MultiIndex case
                    close_prices[t] = raw_data[t]['Close']
                elif 'Close' in raw_data.columns:  # Single ticker case
        ...
        st.warning(f"Data fetch warning: {str(e)}")
        return pd.DataFrame(), pd.Series(), {}
...
try:
    ...
    if prices.empty:
        st.error("❌ No data available for the selected tickers. Please check ticker symbols and try again.")
        st.stop()
    ...
    if len(available_tickers) != len(ticker_list):
        st.warning(f"⚠️ Some tickers were not found: {set(ticker_list) - set(available_tickers)}")
    ...
    1. <strong>Invalid ticker symbols:</strong> Check if all tickers are correct and include exchange codes for international stocks (e.g., MC.PA for LVMH)<br>
    2. <strong>Yahoo Finance rate limits:</strong> Wait 1-2 minutes and try again with fewer tickers<br>
